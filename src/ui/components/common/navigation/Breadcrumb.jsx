...
export default function Breadcrumb({
  crumbs,
  className = "absolute top-4 left-4 sm:translate-x-0 flex items-center ",
  textColor = "base",
}) {
  const color = {
    base: "text-[#228E8D]",
    ...
    <nav className={`${className}`}>
      <ol className="flex space-x-2">
        {crumbs.map((crumb, index) => {
          ...
          return (
            <li key={index} className={`flex items-center ${color[textColor]}`}>
              {!isLast ? (
                <>
                  <Link
                    to={crumb.to}
                    className="hover:opacity-100 opacity-75 font-semibold"
                  >
                    {crumb.label}
                  </Link>
                  <svg
                    ...
                    height="12"
                    viewBox="0 0 7 12"
                    className="fill-current opacity-75 ml-2 mt-[1.5px]"
                  >
